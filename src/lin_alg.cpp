...
Eigen::VectorXd vectorize( Eigen::MatrixXd Q)
{
   ...
   Eigen::VectorXd vQ(n*m);
   vQ.setZero();
   
   for (int j=0; j<m;j++){
     vQ.segment(j*n,n) = Q.col(j);
   }
   return (vQ);
 }
...
Eigen::MatrixXd extract_submatrix(Eigen::MatrixXd Mold, Eigen::VectorXd indin)
{
   ...
   if (m>1){
     for (int a=0;a<n;a++){
       for (int b=0;b<n;b++){
         MM(a,b) = Mold((int)indin[a],(int)indin[b]);
       }
     }
   }
   if (m==1){
